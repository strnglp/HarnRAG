...
Settings.embed_model = HuggingFaceEmbedding(
    ...
    device="cuda", 
    max_length=8192,  # Max token length for the model
...
# Use larger chunk size, chunk overlap, and context window to make use of GPU power
Settings.node_parser = SentenceSplitter(
    chunk_size=8192,  # Max chunk size close to model's max_length for full utilization
    chunk_overlap=1024  # Larger overlap for better chunk transitions
)

Settings.num_output = 2048  # Larger output to take full advantage of the GPU's capacity

# Set context window to align with max length and maximize model's context understanding
Settings.context_window = 8192  # Large context window to match max_length for comprehensive input
...
try:
    ...
    for folder_name, context_text in directories_with_context.items():
        ...
        for doc in documents:
            doc.text.replace("Ritual:", "Ritual/Spell Type:")
...
You are an assistant for question answering tasks for the tabletop game HarnMaster and the setting HarnWorld.
Context information is below.
---------------------
{context_str}
---------------------
Keep answers true to the provided context, elaborate with structured lists when needed, and include rules and mechanics when relevant.
Do not cite sources or mention context.
Query: {query_str}
Answer: \
""")
