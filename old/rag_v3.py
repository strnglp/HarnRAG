...
storage_path = "./storage_v3/" # computed data
...
try:
    ...
    pdf_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith(".pdf") and not f.startswith(".")]
    reader = PyMuPDFReader()
    documents = []
    for pdf_file in pdf_files:
        try:
            doc = reader.load(file_path=pdf_file)
